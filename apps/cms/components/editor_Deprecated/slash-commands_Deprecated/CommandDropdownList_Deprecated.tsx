...
const CommandDropdownList = forwardRef<CommandsListRef, CommandsListProps>(
	(props, ref) => {
						...
						input.onchange = async e => {
							...
							if (file && token) {
								if (file.type === "image/svg+xml") {
									toast.error("Sorry! SVGs are not supported here.")
									return
								}
								// TODO: Compress file here?
								// upload file here
								toast.promise(UploadFile(file, token as string), {
									error: err => {
										return (
										...
										if (res.success) {
											...
											editor?.commands.insertContent({
												...
												attrs: {
													...
													assetId: res?.data?._id as string,
		...
		const handlePageSelection = ({
			...
			editor.chain().focus().deleteRange(range).setPage({ id, slug, name }).run()
		}
				...
				<CustomDialogBox
					...
					<div className="flex flex-col space-y-4">
						...
						<div className="w-full flex items-center justify-end space-x-4">
							...
							<Button onClick={() => handleYoutubeLinkAdd()}>Add</Button>
						</div>
					</div>
				</CustomDialogBox>

				{/* Page Modal */}
				<CustomDialogBox open={pageModalOpen} onOpenChange={setPageModelOpen}>
					<AllPagesList
