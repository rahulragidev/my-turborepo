...
const FeaturedBlogsSectionExtension = TipTapNode.create({
	name: "featured-blogs",
		...
		return {
			setFeaturedPages: (_options?: SetFeaturedPagesOptions) => () => {
				const fetchAndUpdateContent = async () => {
					try {
						const siteId =
							typeof this.editor.options?.editorProps?.attributes ===
							"object"
						...
						console.log("fetchFeaturedBlogs called with", token, siteId)
